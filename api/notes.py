...
@notes.post('/generate')
async def create_note(note: NoteSchemaAdd, note_service: NoteService = Depends(init_note_service)):
    note_id = await note_service.add_note(note)
    return JSONResponse(content={'note_id': jsonable_encoder(note_id)}, status_code=201)


@notes.get('/secrets/{note_id}')
async def get_note(note_id: UUID, note_service: NoteService = Depends(init_note_service)):
